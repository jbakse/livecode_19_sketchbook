...
// went toward realism here with color and shading model.
// might back up and try another way.
...
function* gen() {
  ...
  for (let y = 0; y < 480; y += yRes) {
    for (let x = 0; x <= 720; x += xRes) {
      ...
      // draw mountain edge
      noFill();
      const hue = map(dist(_x1, _y1, _x2, _y2), 1, 5, 0.1, 0.7, true);
      const sat = map(dist(_x1, _y1, _x2, _y2), 1, 5, 0.8, 0.2, true);
      const bright = map((y1 - y2) / (x1 - x2), -1, 1, 0.2, 0.5);

      stroke(hue, sat, bright, 0.2);
      //   strokeWeight(2);
...
function recurve(x, y, n) {
  const stepped = steppe(n, 11);
  const stepped2 = steppe(n, 6);

  return lerp(stepped, stepped2, noise(x * 0.2, y * 0.2));
}
