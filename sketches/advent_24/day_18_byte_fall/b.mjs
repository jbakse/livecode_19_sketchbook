...
/// parse input
const input = await Deno.readTextFile(import.meta.dirname + "/" + config.input);
// console.log("Input");
// console.log(input);
...
for (let bytesFallen = 1; bytesFallen <= locations.length; bytesFallen++) {
  console.log();
  log(`Bytes Fallen: ${bytesFallen}`);
  log(
    `This byte falls at: ${locations[bytesFallen - 1].x},${
      locations[bytesFallen - 1].y
    }`
  );

  const map = array2D(config.width, config.height, ".");
  ...
  const path = walkBack(positions, config.width - 1, config.height - 1);
  if (path === false) {
    error("No path found");
    break;
  }
