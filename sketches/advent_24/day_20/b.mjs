...
const visited = search(map, { x: startPosition.x, y: startPosition.y });

const offsets = manhattanOffsets(2, 20);

let count = 0;
for (const tile of visited) {
  const toX = Number.parseInt(tile[0].split("#")[0]);
  const toY = Number.parseInt(tile[0].split("#")[1]);
  const toCost = tile[1].cost;
  for (const offset of offsets) {
    const fromX = toX + offset.x;
    const fromY = toY + offset.y;
    if (visited.has(`${fromX}#${fromY}`)) {
      const fromCost = visited.get(`${fromX}#${fromY}`).cost;
      const savings =
        toCost - fromCost - (Math.abs(offset.x) + Math.abs(offset.y));
...
function manhattanOffsets(minSize = 0, size = 1) {
  const offsets = [];
  for (let y = -size; y <= size; y++) {
    for (let x = -size; x <= size; x++) {
      if (Math.abs(x) + Math.abs(y) <= size) {
        if (Math.abs(x) + Math.abs(y) >= minSize) {
          offsets.push({ x, y });
        }
      }
    }
