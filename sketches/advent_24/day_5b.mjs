console.log("Advent of Code 2024 - Day 5b");
...
for (const update of updates) {
  ...
  console.log("illegal update", update);
  const sortedUpdate = [...update].sort(sortPageNums);

  console.log("sorted", sortedUpdate);

  // get middle item from update
  const middle = sortedUpdate[Math.floor(sortedUpdate.length / 2)];
  sum += Number.parseInt(middle, 10);
}
console.log("sum", sum);

function sortPageNums(a, b) {
  const forward = rules.find((r) => r[0] === a && r[1] === b);
  if (forward) return -1;
  const reverse = rules.find((r) => r[0] === b && r[1] === a);
  if (reverse) return 1;
  return 0;
}
