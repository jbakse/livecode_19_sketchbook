...
// Javascript provides _prototypical delegation_ as a method for sharing data between objects. In short, each object has an internal link to another object, the linked object is refered to as it's _prototype_. If you try to access a property of an object that doesn't exist, Javascript will look to see if the object's prototype has the property, and use the prototype's property's value if it does.

// When you create an object instance using `new Constructor()` javascript sets the new object's prototype to the Constructor function's prototype.

// The practical effect of this is that you can put shared methods on a Constructor function's prototype, and those methods will be usable by all the instance objects it constructs.
