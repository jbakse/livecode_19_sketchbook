...
class Dot {
  ...
  step(dTime) {
    ...
    let spreadDot = (other) => {
      ...
      let force = map(d, 0, this.r, 3, 1, true);
    ...
    if (this.parent) {
      ...
      if (d > this.r) {
        ...
        //   let force = map(d, 0, this.r, 0, 1);
        //   this.x += cos(a) * -force * 0.1;
        //   this.y += sin(a) * -force * 0.1;

        this.x = this.parent.x + cos(a) * this.r;
        this.y = this.parent.y + sin(a) * this.r;
      }
    }
    ...
    if (dots.length < 800) {
      if (this.age - dTime < this.spawn_time && this.age >= this.spawn_time) {
        ...
        if (this.parent) {
          let a = atan2(this.y - this.parent.y, this.x - this.parent.x);
          target_x = this.x + cos(a) * 0.5;
          target_y = this.y + sin(a) * 0.5;
        }

        spawnNear(target_x, target_y, 0.1, this);
        if (random() < 0.2) {
          spawnNear(target_x, target_y, 0.1, this);
        }
      }
  ...
  draw() {
    ...
    // draw hair
    strokeWeight(map(this.gen, 1, 20, 1, 0.5, true));
    let len = map(this.gen, 1, 20, 10, 2, true);
    if (this.parent) {
      let a = atan2(this.y - this.parent.y, this.x - this.parent.x);
      let a1 = a - noise(this.x * 0.1);
      let a2 = a + noise(this.y * 0.1);

      line(
        this.parent.x,
        this.parent.y,
        this.parent.x + cos(a1) * len,
        this.parent.y + sin(a1) * len
      );
      line(
        this.parent.x,
        this.parent.y,
        this.parent.x + cos(a2) * len,
        this.parent.y + sin(a2) * len
      );

      line(
        lerp(this.parent.x, this.x, 0.5),
        lerp(this.parent.y, this.y, 0.5),
        lerp(this.parent.x, this.x, 0.5) + cos(a1) * len,
        lerp(this.parent.y, this.y, 0.5) + sin(a1) * len
      );
      line(
        lerp(this.parent.x, this.x, 0.5),
        lerp(this.parent.y, this.y, 0.5),
        lerp(this.parent.x, this.x, 0.5) + cos(a2) * len,
        lerp(this.parent.y, this.y, 0.5) + sin(a2) * len
      );
    }
