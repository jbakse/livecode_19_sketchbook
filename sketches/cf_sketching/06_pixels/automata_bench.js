...
let ppf = document.createElement("div");
...
function avg(a) {
    return a.reduce((acc, v) => acc + v) / a.length;
...
function draw() {
    ...
    for (let i = 0; i < g.width * g.height * 100; i++) {
        if (performance.now() - start > 30) {
            ppf_counts.push(i);
            if (ppf_counts.length > 60) {
                ppf_counts.shift();
            }

            ppf.textContent = "ppf: " + Math.floor(avg(ppf_counts));// + " " + Math.floor(frameRate());
            //console.log(pixels_placed);
            break;
        }
