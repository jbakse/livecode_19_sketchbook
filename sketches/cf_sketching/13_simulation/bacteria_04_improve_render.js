...
class Bacterium {
  ...
  step(t) {
    ...
    if (this.spring.length < 10) {
      this.spring.length += 0.07 * t * this.speed;
    } else {
      splitBacterium(this);
    }
  }
...
function splitBacterium(old) {
  if (particles.length > 4000) return;
...
function render() {
  ...
  if (DRAW_RENDER) {
    ...
    filter(BLUR, 1);
    noStroke();
    fill(noise(frameCount * 0.2, 1) * 50 + 150);
    textSize(20);
    textFont("monospace");
    textAlign(RIGHT);
    text(`${floor(frameCount / 4)} min`, 100, 460);
