...
view.onFrame = (e) => {
  const v = plantGenerator.next().value;
...
function* createPlants() {
  ...
  const center = new Point(WIDTH * 0.5, HEIGHT * 0.5);
  const points = clusterPoints(center, PLANT_COUNT, PLANT_SPACING);
  ...
  points.sort((a, b) =>
    a.subtract(center).length < b.subtract(center).length ? 1 : -1
  );
...
function createPlant() {
  ...
  const plant_type = pick(plant_types);

  // create a cluster of points for the leaves
  const points = clusterPoints(new Point(0, 0), LEAF_COUNT, LEAF_SPACING);

  // sort by distance from 0,0 so we draw the middle parts after the outer parts
  points.sort((a, b) => (a.length < b.length ? 1 : -1));
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const part_function = pick(plant_configs[plant_type]);
    // make the part
    const part = part_function(point, LEAF_RADIUS);
    part.name = `part_${i}`;
    parts.addChild(part);
  }
...
function createFlower(center, radius) {
  let position = center;
  let offset = center.multiply(0.25);

  const flower_group = new Group();
  const blocker_group = new Group();
  for (let i = 0; i < 5; i++) {
    const r = map(i, 0, 5, radius, radius * 0.25);
    const circle = createCircle(position, r);
    flower_group.addChild(circle);

    const blocker = new Path.Circle(position, r * 0.8);
    blocker.style = {
      fillColor: "white",
...
function createVine(center, radius) {
  const length = pick([8, 8, 8, 9, 9, 10, 20, 30, 40]);
  let offset_x = 0;

  const vine_group = new Group();
  for (let i = 0; i < length; i++) {
    const offset_point = [offset_x, i * radius * 0.6];
    offset_x += randomRange(-radius, radius) * 0.5;
    offset_x *= 0.5;
    const circle = createCircle(center.add(offset_point), radius * 0.4);
...
function createCircleDrawn(center, radius) {
  ...
  const back_path = new Path.Circle(center, radius + GAPPY);
  back_path.name = "back";
  ...
  const path = new Path.Circle(center, radius);
  path.name = "front";
  //   var pressure = randomRange(-0.5, 0.5);
  path.style = {
    fillColor: "white",
...
function debugPoint(point, radius) {
  const path = new Path.Circle(point, radius);
  path.style = {
...
function downloadAsSVG(fileName) {
  ...
  const svgData = project.exportSVG({ asString: true });
