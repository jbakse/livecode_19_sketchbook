...
void main() {
    ...
    float foreground = gl_FragCoord.x / 600.0 + (gl_FragCoord.y - 400.0) / 200.0;
    foreground = 1.0 - abs(foreground - 0.5) * 2.0;
    foreground = clamp(foreground, 0.0, 1.0);
    foreground = pow(foreground, 4.0);
    float g = 1.-mix(foreground, background, step(200.0, d));
    gl_FragColor = vec4(g, g, g, 1.0);
    // gl_FragColor = vec4(foreground, foreground, foreground, 1.0);
}
