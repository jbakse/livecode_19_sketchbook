...
vec2 polarize(vec2 uv, vec2 center) {
    // uv += vec2(u_time * 0.1, 0.0);
    // uv.y *= 0.4;
    
    //vec2 polar = vec2(sin(uv.x * 3.14) * uv.y, cos(uv.x * 3.14) * uv.y);
    
    vec2 ray = normalize(uv - center);
    vec2 up = vec2(0.0, 1.0);
    float a = acos(dot(up, ray));
...
void main() {
    ...
    uv = polarize(uv, vec2(u_resolution.x / u_resolution.y * 0.5, 0.5));
    uv.x += u_time * 0.01 + sin(u_time) * 0.04;
    uv.y += u_time * -0.3 + sin(u_time) * 0.05;
