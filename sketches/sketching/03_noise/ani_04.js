...
window.draw = function () {
  ...
  const w = roundTo(width - 2 * margin, 8);
  const h = roundTo(height - 2 * margin, 8);
...
function weaveRect(x, y, w, h, c, solid, messy) {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    for (let col = 0; col < cols + wide; col++) {
      ...
      translate(x + xx, y + yy);
      const sampleX = (x + xx) * 0.005;
      const sampleY = (y + yy) * 0.07;
      translate(noise(sampleX, sampleY, 0) * 8, noise(sampleX, sampleY, 1) * 4);

      rotate(random(-PI, PI) * 0.01 * messy);
      translate(random(messy * 0.02), random(messy * 0.02));

      rect(0, 0, col_width, row_height, 1, 1, 1, 1);
      ...
      // thread mid line
      stroke(1, 0, 1, 0.1);
      line(0, 5, col_width, 4);
      stroke(1, 0, 0, 0.05);
      line(0, 4, col_width, 3);

      // i had a bug that drew diagonals on the weave rects
      // i liked it, so lets put it in on purpose, rarely.
      if (random() < 0.01) {
        stroke(1, 0, 1, lowRandom(0.5));
        // i had randomized the angle for minute
        // but this works better if its the same everytime it appears
        // a lot of mechanical processes create repeated/consistent mistakes.
        line(col_width, 0, 0, 4);
      }
